...
async function generateSyntheticDataset(rows: number, filepath: string): Promise<void> {
  ...
  const countries = ['United Kingdom', 'France', 'Germany', 'EIRE', 'Spain', 'Netherlands', 'Belgium'];
  ...
  for (let i = 0; i < rows; i++) {
    ...
    lines.push(`${invoiceNo},${stockCode},"${desc}",${quantity},${date},${unitPrice},${customerID},${country}\n`);
  }
