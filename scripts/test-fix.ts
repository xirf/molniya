...
function measureMemoryAndTime<T>(fn: () => Promise<T>, label: string): Promise<T> {
	return new Promise(async (resolve, reject) => {
		...
		try {
			...
			console.log(`${label}: ${duration.toFixed(2)}ms, memory: ${memoryDelta.toFixed(2)}MB`);
			resolve(result);
		} catch (error) {
			reject(error);
		}
	...
	await measureMemoryAndTime(async () => {
		...
		for await (const chunk of csvSource) {
			...
			if (chunks % 100 === 0) {
				...
				console.log(`  Processed ${chunks} chunks, ${rows} rows, memory: ${memory.toFixed(2)}MB`);
			}
		}
