...
export class BinaryReader {
    ...
    private deserializeColumn(
        ...
        let colBuffer: ColumnBuffer;
        if (kind === DTypeKind.StringView) {
            colBuffer = new StringViewColumnBuffer(rowCount, nullable) as unknown as ColumnBuffer;
        } else {
            colBuffer = new ColumnBuffer(kind, rowCount, nullable);
        }
        ...
        if (kind === DTypeKind.String) {
        ...
        } else if (kind === DTypeKind.StringView) {
            const offsetsLen = (rowCount + 1) * 4;
            ...
            const svCol = colBuffer as unknown as StringViewColumnBuffer;
            for (let i = 0; i < rowCount; i++) {
                if (nullable && nullBitmap) {
                    const isNull = (nullBitmap[i >>> 3]! & (1 << (i & 7))) !== 0;
                    if (isNull) {
                ...
                const strBytes = buffer.subarray(stringDataStart + start, stringDataStart + end);
                svCol.appendBytes(strBytes, 0, strBytes.length);
            }
        } else {
