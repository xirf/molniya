...
function logMemory(label: string) {
  ...
  console.log(
    `   [Memory] ${label}: ${mem.heapUsed.toFixed(1)}MB heap, ${mem.rss.toFixed(1)}MB RSS`,
  );
}
...
async function main() {
  ...
  if (hasErrors && parseErrors) {
    ...
    for (const [col, failures] of parseErrors) {
      console.log(
        `   ${String(col)}: ${failures.failureCount} failures (${(failures.successRate * 100).toFixed(2)}% success)`,
      );
    }
  }
  console.log('');
  ...
  console.log(
    `   That's ${((highPriceMoments.shape[0] / df.shape[0]) * 100).toFixed(2)}% of the dataset`,
  );
  console.log('');
  ...
  for (const range of priceRanges) {
    ...
    console.log(
      `   ${range.label.padEnd(20)}: ${count.toLocaleString().padStart(12)} minutes (${pct}%)`,
    );
  }
  console.log('');
